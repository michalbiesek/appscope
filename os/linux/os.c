...
osSwitchNamespace(pid_t hostPid)
{
  ...
  scope_fprintf(stderr, "osSwitchNamespace size %d\n", nsSize);

  for (i = 0; i < nsSize; ++i) {
    if (scope_snprintf(nsPath, sizeof(nsPath), "/proc/%d/ns/%s", hostPid, nsType[i]) < 0) {
        scope_fprintf(stderr, "snprintf failed\n");
        return FALSE;
    }
    int nsFd = scope_open(nsPath, O_RDONLY);
    if (!nsFd) {
        scope_fprintf(stderr, "open %s failed %d\n", nsPath, scope_errno);
        return FALSE;
    }

    if (scope_setns(nsFd, 0) != 0) {
        scope_fprintf(stderr, "setns failed %d\n", scope_errno);
        return FALSE;
    }
  }
