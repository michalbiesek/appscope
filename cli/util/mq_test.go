...
func TestMqNoOverwriteReader(t *testing.T) {
	const mqName string = "goTestMqReader"

	msgQ, err := newMsgQReader(mqName)
	assert.NoError(t, err)
	assert.NotNil(t, msgQ)
	msgQSecond, err := newMsgQReader(mqName)
	assert.Error(t, err)
	assert.Nil(t, msgQSecond)
	err = msgQ.destroy()
	assert.NoError(t, err)
}

func TestMqNoOverwriteWriter(t *testing.T) {
	const mqName string = "goTestMqWriter"

	msgQ, err := newMsgQWriter(mqName)
	assert.NoError(t, err)
	assert.NotNil(t, msgQ)
	msgQSecond, err := newMsgQWriter(mqName)
	assert.Error(t, err)
	assert.Nil(t, msgQSecond)
	err = msgQ.destroy()
	assert.NoError(t, err)
