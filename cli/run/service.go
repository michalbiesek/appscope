...
func (rc *Config) Service(serviceName string, user string, force bool, quiet bool) error {
	...
	if util.CheckFileExists("/etc/rc.conf") {
		// OpenRC
		return rc.installOpenRc(serviceName, unameMachine, unameSysname, libcName, force, quiet)
	} else if util.CheckDirExists("/etc/systemd") {
		// Systemd
		return rc.installSystemd(serviceName, unameMachine, unameSysname, libcName, force, quiet)
	} else if util.CheckDirExists("/etc/init.d") {
		// Initd
		return rc.installInitd(serviceName, unameMachine, unameSysname, libcName, force, quiet)
	}
...
func (rc *Config) installScope(serviceName string, unameMachine string, unameSysname string, libcName string) (string, error) {
	...
	if !util.CheckDirExists(configBaseDir) {
		...
		if err != nil {
			return libraryPath, fmt.Errorf("error: failed to create config base directory; %v", err)
		}
	}
...
func (rc *Config) installSystemd(serviceName string, unameMachine string, unameSysname string, libcName string, force bool, quiet bool) error {
...
func (rc *Config) installInitd(serviceName string, unameMachine string, unameSysname string, libcName string, force bool, quiet bool) error {
...
func (rc *Config) installOpenRc(serviceName string, unameMachine string, unameSysname string, libcName string, force bool, quiet bool) error {
	initScript := "/etc/init.d/" + serviceName
	if !util.CheckFileExists(initScript) {
		return errMissingService
	}
