...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// If there is no port skip the setup ExporterListenerType
		if i != -1 {
			port, err := strconv.Atoi(exporterDest[i+1:])
			// If converstion fails skip setup ExporterListenerType
			if err == nil {
				opt.ExporterStatsDPort = port
				if strings.HasPrefix(exporterDest, "tcp://") {
					opt.ExporterListenerType = "tcp"
				} else if strings.HasPrefix(exporterDest, "udp://") {
					opt.ExporterListenerType = "udp"
				}
			}
		}
