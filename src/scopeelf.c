...
getElf(char *path)
{
    ...
    if ((ebuf = mm_calloc(1, sizeof(elf_buf_t))) == NULL) {
    ...
    char * mmap_rv = mm_mmap(NULL, ROUND_UP(sbuf.st_size, sysconf(_SC_PAGESIZE)),
                          PROT_READ, MAP_PRIVATE, fd, (off_t)NULL);
    if (mmap_rv == MAP_FAILED) {
        scopeLogError("fd:%d getElf: mm_mmap failed", fd);
        goto out;
    }
