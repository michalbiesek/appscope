...
static size_t
cmdGetAttachStatus(char *buf, size_t len) {
    ...
    #endif
    return scope_snprintf(buf, len, "[pid %d] %s", scope_getpid(), status);
}
...
ipcRequestMsgHandler(mqd_t mqDes, size_t mqSize, ipc_cmd_t *cmdRes) {
    ...
    input_cmd_table_t inputCmdTable[] = {
        {"getAttachStatus",  INPUT_CMD_LEN("getAttachStatus"), IPC_CMD_GET_ATTACH_STATUS}
    };
...
ipcResponseMsgHandler(mqd_t mqDes, size_t mqSize, ipc_cmd_t cmd) {
    bool res = FALSE;
    size_t len;
    // TODO cast enum to ensure it is unsigned ?
    ...
    output_cmd_table_t outputCmdTable[] = {
        {IPC_CMD_GET_ATTACH_STATUS, cmdGetAttachStatus},
