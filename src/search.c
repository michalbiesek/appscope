...
searchComp(const char *input_str)
{
    ...
    search_t* handle = mm_calloc(1, sizeof(search_t));
    if (!handle) goto failed;
    handle->nlen = strlen(input_str);
    if (!handle->nlen) goto failed;
    handle->str = (unsigned char*)mm_strdup(input_str);
...
searchFree(search_t **handle_ptr)
{
    ...
    if (handle && handle->str) mm_free(handle->str);
    if (handle) mm_free(handle);
    *handle_ptr = NULL;
}
