...
sendProcessStartMetric()
{
    ...
    if (urlEncodedCmd) mm_free(urlEncodedCmd);
}
                    ...
                    pcre2_match_data *match_data, pcre2_match_context *mcontext)
{
    ...
    if ((pcre_stack = mm_malloc(PCRE_STACK_SIZE)) == NULL) {
        scopeLogError("ERROR; pcre2_match_wrapper: mm_malloc");
        return -1;
    }
    ...
    if (pcre_stack) mm_free(pcre_stack);
    return rc;
#else
    return pcre2_match(re, data, size, startoffset, options, match_data, mcontext);
#endif
                ...
                regmatch_t *pmatch, int eflags)
{
   if (g_need_stack_expand == FALSE) {
    ...
    char *pcre_stack = NULL, *tstack = NULL, *gstack = NULL;
     ...
     if ((pcre_stack = mm_malloc(PCRE_STACK_SIZE)) == NULL) {
        scopeLogError("ERROR; regexec_wrapper: mm_malloc");
        return -1;
    }
