...
get_dir(const char *path, char *fres, size_t len)
{
    ...
    pcopy = mm_strdup(path);
    dname = dirname(pcopy);

    if ((dirp = opendir(dname)) == NULL) {
        perror("get_dir:opendir");
        if (pcopy) mm_free(pcopy);
        return res;
    }

    dcopy = mm_strdup(path);
...
set_library(const char *libpath)
{
    ...
    buf = mm_mmap(NULL, ROUND_UP(sbuf.st_size, sysconf(_SC_PAGESIZE)),
               PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, (off_t)NULL);
    if (buf == MAP_FAILED) {
        perror("set_loader:mm_mmap");
        close(fd);
        return -1;
    }
...
main(int argc, char **argv, char **env)
{
    ...
    // build exec args
    int execArgc = 0;
    char **execArgv = mm_calloc(argc + 4, sizeof(char *));
    if (!execArgv) {
        perror("mm_calloc");
        return EXIT_FAILURE;
    }
