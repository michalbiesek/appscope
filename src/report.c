 ...
 /*
    ...
    */
    if (proto->ptype == EVT_HRES) {
        ...
        if ((mtcEnabled(g_mtc)) && (cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_HTTP))) {
              ...
              const char *func, const char *name, void* ctr)
{
    ...
    case NET_ERR_RX_TX:
    {
        ...
        if ((!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_NET)) ||
    ...
    case NET_ERR_DNS:
    {
        ...
        // Only report if metrics enabled
        if ((!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_FS)) ||
...
doDNSMetricName(metric_t type, net_info *net)
{
    ...
    case DNS:
    {
        ...
        // Only report if metrics enabled
        if ((g_summary.net.dns) || (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_DNS))) {
            return;
        }
...
doStatMetric(const char *op, const char *pathname, void* ctr)
{
    ...
    if ((g_summary.fs.stat) || !cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_FS)) {
        return;
    }
           ...
           const char *op, ssize_t size, const char *pathname)
{
    ...
    case FS_DURATION:
    {
        ...
        if (((g_summary.fs.read_write) && (source == EVENT_BASED)) ||
           (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_FS))) {
            return;
        }
...
doTotal(metric_t type)
{
    ...
    switch (type) {
        case TOT_READ:
            if (!g_summary.fs.read_write || (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_FS)))
                return;
            metric = "fs.read";
        ...
        case TOT_RX:
        case TOT_TX:
            if (!g_summary.net.rx_tx || (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_NET)))
                return;
            doTotalNetRxTx(type);
            return;   // <--  We're doing the work above; nothing to see here.
        case TOT_SEEK:
            if (!g_summary.fs.seek || (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_FS)))
                return;
            metric = "fs.seek";
        ...
        case TOT_STAT:
            ...
            break;
        case TOT_OPEN:
            if (!g_summary.fs.open_close || (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_FS)))
                return;
            metric = "fs.open";
        ...
        case TOT_DNS:
            ...
            break;
        case TOT_PORTS:
            if (!g_summary.net.open_close || (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_NET)))
                return;
            metric = "net.port";
...
doNetMetric(metric_t type, net_info *net, control_type_t source, ssize_t size)
{
    ...
    case NET_CONNECTIONS:
    {
        ...
        if (((g_summary.net.open_close) && (source == EVENT_BASED)) ||
           (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_NET))) {
            return;
        }
...
doHttpAgg()
{
    if (cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_HTTP)) {
...
{
    if (!cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_PROC)) {
        return;
    }
