...
{
    list_t *list = mm_calloc(1, sizeof(list_t));
    list_element_t* head = mm_calloc(1, sizeof(list_element_t));
    if (!list || !head) {
        if (list) mm_free(list);
        if (head) mm_free(head);
        return NULL;
    }
    list->head = head;
...
lstInsert (list_t *list, list_key_t key, void* data)
{
    ...
    list_element_t *new_node = mm_calloc(1, sizeof(list_element_t));
...
lstDelete (list_t *list, list_key_t search_key)
{
    ...
    if (right_node) mm_free(right_node);
