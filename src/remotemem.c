...
static const void*
resolve_vaddr(unsigned long vaddr, vector *mapping)
{
    for (unsigned i = 0; i < mapping->size; ++i) {
        mem_region_t *region = vecGet(mapping, i);
...
static int
locate_program_headers(vector *mapping, program_headers_t *ph) {
    
    mem_region_t *reg = vecGet(mapping, 0);
...
static int
locate_dynamic_section(vector *mapping, dynamic_section_t *dynamic) {
    ...
    int resH = locate_program_headers(mapping, &ph);
...
static int locate_dynstr_section(dynamic_section_t *dynamic, vector *mapping, dynstr_section_t *dynstr)
{
    dynstr->vaddr = 0;
    dynstr->size = 0;
    ...
    for (unsigned long i = 0; i < dynamic->count; i++) {
		...
		if (dynamic->entries[i].d_tag == DT_PLTGOT) {
			scope_fprintf(scope_stderr, "DT_PLTGOT found");
		}

		if (dynamic->entries[i].d_tag == DT_DEBUG) {
			scope_fprintf(scope_stderr, "DT_DEBUG found");
		}
...
static int locate_dynsym_section(dynamic_section_t *dynamic, vector *mapping, dynsym_section_t *dynsym)
{
    dynsym->vaddr = 0;
    dynsym->syment = 0;
...
static int
locate_hash_section(dynamic_section_t *dynamic, vector *mapping, hash_section_t *hash) {
    hash->vaddr = 0;
...
findDynamicSymbolTable(vector *memLibMap) {
    if (memLibMap->size < 1) {
        return NULL;
    }

    const mem_region_t *begin = vecGet(memLibMap, 0);
    ...
    if (locate_dynamic_section(memLibMap, &dynamic) < 0) {
    ...
    if (locate_dynstr_section(&dynamic, memLibMap, &dynstr) < 0) {
        scope_fprintf(scope_stderr, "\nlocate_dynstr_section failed");
        return NULL;
    }
    if (locate_dynsym_section(&dynamic, memLibMap, &dynsym) < 0) {
        scope_fprintf(scope_stderr, "\nlocate_dynsym_section failed");
        return NULL;
    }
    if (locate_hash_section(&dynamic, memLibMap, &hash) < 0) {
