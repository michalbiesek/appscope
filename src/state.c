    ...
    if ((net && net->tlsDetect == DETECT_FALSE) || (src == TLSTX || src == TLSRX)) {
        ...
        if (net && net->protoProtoDef) {
            ...
            if ((!scope_strcasecmp(net->protoProtoDef->protname, "HTTP")) &&
                ((cfgEvtFormatSourceEnabled(g_cfg.staticfg, CFG_SRC_HTTP)) || (cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_HTTP)))) {
                doHttp(id, sockfd, net, buf, len, src, dtype);
            }

            if (cfgMtcCategoryEnable(g_cfg.staticfg, CFG_MTC_STATSD) &&
