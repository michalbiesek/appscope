...
// This function returns a pointer to a mm_malloc()'d buffer.
// The caller is responsible for deallocating with mm_free().
...
// Helper functions - returns a pointer to a mm_malloc()'d buffer.
// The caller is responsible for deallocating with mm_free().
