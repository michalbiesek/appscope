...
{ 
    ...
    SCOPE_BIT_SET_VAR(c->mtc.category_enable, CFG_MTC_FS, DEFAULT_MTC_FS_ENABLE);
    SCOPE_BIT_SET_VAR(c->mtc.category_enable, CFG_MTC_NETWORK, DEFAULT_MTC_NETWORK_ENABLE);
    SCOPE_BIT_SET_VAR(c->mtc.category_enable, CFG_MTC_HTTP, DEFAULT_MTC_HTTP_ENABLE);
    SCOPE_BIT_SET_VAR(c->mtc.category_enable, CFG_MTC_DNS, DEFAULT_MTC_DNS_ENABLE);
    SCOPE_BIT_SET_VAR(c->mtc.category_enable, CFG_MTC_PROC, DEFAULT_MTC_PROC_ENABLE);
...
{
    return (cfg) ? SCOPE_BIT_CHECK(cfg->mtc.category_enable, CFG_MTC_FS) : DEFAULT_MTC_ENABLE;
}

unsigned
cfgMtcNetEnable(config_t* cfg)
{
    return (cfg) ? SCOPE_BIT_CHECK(cfg->mtc.category_enable, CFG_MTC_NETWORK) : DEFAULT_MTC_NETWORK_ENABLE;
}

unsigned
cfgMtcHttpEnable(config_t* cfg)
{
    return (cfg) ? SCOPE_BIT_CHECK(cfg->mtc.category_enable, CFG_MTC_HTTP) : DEFAULT_MTC_HTTP_ENABLE;
}

unsigned
cfgMtcDnsEnable(config_t* cfg)
{
    return (cfg) ? SCOPE_BIT_CHECK(cfg->mtc.category_enable, CFG_MTC_DNS) : DEFAULT_MTC_DNS_ENABLE;
}

unsigned
cfgMtcProcEnable(config_t* cfg)
{
    return (cfg) ? SCOPE_BIT_CHECK(cfg->mtc.category_enable, CFG_MTC_PROC) : DEFAULT_MTC_PROC_ENABLE;
}
...
cfgMtcCategoryEnableSet(config_t *cfg, unsigned val, metric_category_t type)
{
    ...
    switch (type) {
        ...
        case CFG_MTC_DNS:
        case CFG_MTC_PROC:
            SCOPE_BIT_SET_VAR(cfg->mtc.category_enable, type, val);
            break;
        default:
            DBG(NULL);
    }
