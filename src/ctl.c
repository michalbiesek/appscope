...
grab_supplemental_for_def_protocol(cJSON * json_root, request_t *req)
{
    ...
    if ((prot = mm_calloc(1, sizeof(protocol_def_t))) == NULL) goto err;
    ...
    if (!json) goto err;
    if (!(str = cJSON_GetStringValue(json))) goto err;
    prot->regex = mm_strdup(str);

    json = cJSON_GetObjectItem(body, "pname");
    if (!json) goto err;
    if (!(str = cJSON_GetStringValue(json))) goto err;
    prot->protname = mm_strdup(str);

    return;

err:
    if (req) req->cmd=REQ_PARAM_ERR;
    if (prot && prot->regex) mm_free(prot->regex);
    if (prot && prot->protname) mm_free(prot->protname);
    if (prot) mm_free(prot);
}
...
ctlParseRxMsg(const char *msg)
{
    ...
    request_t *req = mm_calloc(1, sizeof(request_t));
    if (!req) {
        DBG(NULL);
        goto out;
    }
...
destroyReq(request_t **request)
{
    ...
    if (req->cmd_str) mm_free(req->cmd_str);
...
prepMessage(upload_t *upld)
{
    ...
    char *temp = mm_realloc(streamMsg, strsize+2); // room for "\n\0"
    if (!temp) {
        DBG(NULL);
        scopeLogInfo("CTL mm_realloc error");
        mm_free(streamMsg);
        return NULL;
    }
...
ctlCreate()
{
    ctl_t *ctl = mm_calloc(1, sizeof(ctl_t));
    if (!ctl) {
        DBG(NULL);
        goto err;
    }
    ...
    ctl->payload.dir = (DEFAULT_PAYLOAD_DIR) ? mm_strdup(DEFAULT_PAYLOAD_DIR) : NULL;
...
ctlDestroy(ctl_t **ctl)
{
    ...
    if ((*ctl)->payload.dir) mm_free((*ctl)->payload.dir);
...
ctlSendHttp(ctl_t *ctl, event_t *evt, uint64_t uid, proc_id_t *proc)
{
    ...
    if (streamMsg) mm_free(streamMsg);
    return rc;
}
...
{
    log_event_t *event = mm_calloc(1, sizeof(*event));
    char *data = mm_malloc(count);
    char *src = mm_strdup(path);

    if (!event || !data || !path) {
        DBG("event = %p, data = %p, src = %p", event, data, src);
        if (event) mm_free(event);
...
destroyInternalLogEvent(log_event_t **eventptr)
{
    ...
    if (event->id.path) mm_free(event->id.path);
    if (event->data)    mm_free(event->data);
    if (event)          mm_free(event);
    *eventptr = NULL;
}
