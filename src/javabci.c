...
addUtf8Tag(java_class_t *info, const char *str) 
{
    ...
    unsigned char *utf8Tag = mm_malloc(bufsize);
...
javaAddNameAndTypeTag(java_class_t *info, const char *name, const char *desc) 
{
    ...
    size_t bufsize = 5;
    unsigned char *tag = mm_malloc(bufsize);
...
javaCopyMethod(java_class_t *info, unsigned char *method, const char *newName) 
{
    ...
    unsigned char *dest = mm_malloc(len);
...
javaConvertMethodToNative(java_class_t *info, int methodIndex) 
{
    ...
    size_t bufsize = 8;
    unsigned char *addr        = mm_malloc(bufsize);
...
javaAddField(java_class_t *info, const char* name, const char* descriptor, uint16_t accessFlags)
{
    ...
    size_t bufsize = 4 * 2;
    unsigned char *buf = mm_malloc(bufsize);
    info->fields_count++;
...
{
    java_class_t *classInfo = mm_malloc(sizeof(java_class_t));
    ...
    classInfo->constant_pool        = (unsigned char **) mm_calloc(100 + (classInfo->constant_pool_count - 1), sizeof(unsigned char *));
    int i;
    ...
    classInfo->fields               = (unsigned char **) mm_calloc(100 + classInfo->fields_count, sizeof(unsigned char *));
    ...
    classInfo->methods              = (unsigned char **) mm_calloc(100 + classInfo->methods_count, sizeof(unsigned char *));
    ...
    classInfo->attributes           = (unsigned char **) mm_calloc(classInfo->attributes_count, sizeof(unsigned char *));
...
void javaDestroy(java_class_t **classInfo) {
    ...
    for(i = (*classInfo)->_constant_pool_count - 1;i<(*classInfo)->constant_pool_count - 1;i++) {
        mm_free((*classInfo)->constant_pool[i]);
    }
    for(i = (*classInfo)->_methods_count;i<(*classInfo)->methods_count;i++) {
        mm_free((*classInfo)->methods[i]);
    }
    mm_free((*classInfo)->constant_pool);
    mm_free((*classInfo)->fields);
    mm_free((*classInfo)->methods);
    mm_free((*classInfo)->attributes);
    mm_free(*classInfo);
    *classInfo = NULL;
}
