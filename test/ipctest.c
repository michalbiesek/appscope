...
ipcHandlerRequestValid(void **state) {
    ...
    // Put valid message on queue (cmdGetAttachStatus)
    status = scope_mq_send(mqReadWriteDes, "getAttachStatus", scope_strlen("getAttachStatus"), 0);
    assert_int_equal(status, 0);
    ...
    res = ipcRequestMsgHandler(mqReadWriteDes, attr.mq_msgsize, &cmd);
    assert_true(res);
    assert_int_equal(cmd, IPC_CMD_GET_ATTACH_STATUS);
...
ipcHandlerResponseFail(void **state) {
    ...
    res = ipcResponseMsgHandler(mqReadDes, attr.mq_msgsize, IPC_CMD_GET_ATTACH_STATUS);
    assert_false(res);
    msgCount = ipcInfoMsgCount(mqReadDes);
    assert_int_equal(msgCount, 0);
...
ipcHandlerResponseValid(void **state) {
    ...
    res = ipcResponseMsgHandler(mqReadWriteDes, attr.mq_msgsize, IPC_CMD_GET_ATTACH_STATUS);
    assert_true(res);
    ...
    scope_snprintf(respBuf, sizeof(respBuf), "[pid %d] false", scope_getpid());
    assert_int_equal(dataLen, scope_strlen(respBuf));

    //below should reflect cmdGetAttachStatus
    status = scope_memcmp(buf, respBuf, dataLen);
    assert_int_equal(status, 0);
