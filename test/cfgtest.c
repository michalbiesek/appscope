...
verifyDefaults(config_t* config)
{
    ...
    assert_int_equal       (cfgMtcCategoryEnable(config, CFG_MTC_FS), DEFAULT_MTC_FS_ENABLE);
    assert_int_equal       (cfgMtcCategoryEnable(config, CFG_MTC_NET), DEFAULT_MTC_NET_ENABLE);
    assert_int_equal       (cfgMtcCategoryEnable(config, CFG_MTC_HTTP), DEFAULT_MTC_HTTP_ENABLE);
    assert_int_equal       (cfgMtcCategoryEnable(config, CFG_MTC_DNS), DEFAULT_MTC_DNS_ENABLE);
    assert_int_equal       (cfgMtcCategoryEnable(config, CFG_MTC_PROC), DEFAULT_MTC_PROC_ENABLE);
    ...
    assert_int_equal       (cfgMtcCategoryEnable(config, CFG_MTC_STATSD), DEFAULT_MTC_STATSD_ENABLE);
...
cfgMtcCategoryEnableSetAndGet(void** state)
{
    ...
    for (category=CFG_MTC_FS; category<=CFG_MTC_STATSD; ++category) {
        ...
        assert_int_equal(cfgMtcCategoryEnable(config, category), FALSE);
        cfgMtcCategoryEnableSet(config, TRUE, category);
        assert_int_equal(cfgMtcCategoryEnable(config, category), TRUE);
        // 2 is outside of allowed range; should be ignored.
        cfgMtcCategoryEnableSet(config, 2, category);
        assert_int_equal(cfgMtcCategoryEnable(config, category), TRUE);
    }

    cfgMtcCategoryEnableSet(config, FALSE, CFG_MTC_STATSD+1);;
