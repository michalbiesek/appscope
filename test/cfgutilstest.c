...
{
    config_t *cfg = cfgCreateDefault();
    cfgMtcCategoryEnableSet(cfg, TRUE, CFG_MTC_STATSD);
    assert_int_equal(cfgMtcCategoryEnable(cfg, CFG_MTC_STATSD), TRUE);

    // should override current cfg
    assert_int_equal(setenv("SCOPE_METRIC_STATSD", "false", 1), 0);
    cfgProcessEnvironment(cfg);
    assert_int_equal(cfgMtcCategoryEnable(cfg, CFG_MTC_STATSD), FALSE);
...
cfgReadGoodYaml(void** state)
{
    ...
    assert_int_equal(cfgMtcCategoryEnable(config, CFG_MTC_STATSD), FALSE);
...
cfgReadGoodJson(void** state)
{
    ...
    assert_int_equal(cfgMtcCategoryEnable(config, CFG_MTC_STATSD), TRUE);
