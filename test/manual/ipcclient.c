...
bool
getLastPidNamesSpace(int pid, int *lastNsPid) {
...
int main(int argc, char **argv) {
    ...
    if (ipcSwitch) {
        ...
        if (!getLastPidNamesSpace(pid, &nsPid)) {
            return res;
        }
        snprintf(WriteMqName, sizeof(WriteMqName), "/ScopeIPC.%d", nsPid);
    } else {
        snprintf(WriteMqName, sizeof(WriteMqName), "/ScopeIPC.%d", pid);
    }
